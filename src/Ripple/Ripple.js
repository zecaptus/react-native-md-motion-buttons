...
import { StyleSheet, Animated, View, TouchableWithoutFeedback, TouchableNativeFeedback, Platform } from 'react-native';
...
export default class Ripple extends Component {
    ...
    renderCompatibleNative() {
        ...
        return (<TouchableNativeFeedback onPress={this.onPress} background={TouchableNativeFeedback.Ripple(rippleColor)}>
    ...
    renderDefault() {
        ...
        return (<TouchableWithoutFeedback onPressIn={this.onPressIn} onPressOut={this.onPressOut} onPress={this.onPress}>
            <Animated.View style={style} pointerEvents='box-only'>
                <Animated.View style={[StyleSheet.absoluteFill, {backgroundColor:rippleColor, opacity, borderRadius, overflow:'hidden'}]} />
                <View style={[StyleSheet.absoluteFill, {borderRadius, overflow:'hidden'}]}>
                    {ripples.map(this.renderRipple)}
                    {Platform.OS === 'android' && masked ? this.renderAndroidRoundedMask() : null}
                </View>
    ...
    render() {
        return (Platform.OS === 'android' && Platform.Version > 21) ? this.renderCompatibleNative() : this.renderDefault();
    }
}